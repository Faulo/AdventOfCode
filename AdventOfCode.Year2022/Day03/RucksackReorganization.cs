...
class RucksackReorganization {
    ...
    internal static (string, string) DivideIntoCompartments(string line) {
        return (line[..(line.Length / 2)], line[(line.Length / 2)..]);
    }

    internal static char FindLetterThatAppearsInBothCompartments(string line) {
    ...
    internal static int GetLetterPriority(char letter) {
        return letter is >= 'a' and <= 'z'
            ? letter - 'a' + 1
            : letter - 'A' + 27;
    }

    internal static int SumOfPriorityOfLettersThatAppearInBothCompartmentsOfFile(string file) {
    ...
    internal static char FindGroupLetter(string first, string second, string third) {
        for (int i = 0; i < first.Length; i++) {
            char letter = first[i];
            for (int j = 0; j < second.Length; j++) {
                if (letter == second[j]) {
    ...
    internal static IEnumerable<(string first, string second, string third)> DivideIntoGroups(string file) {
    ...
    internal static int SumOfPriorityOfGroup(string file) {
